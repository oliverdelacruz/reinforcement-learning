...
AgentOutputICM = collections.namedtuple('AgentOutputICM', 'icm_inverse icm_forward')
...
class Agent(snt.RNNCore):
    ...
    def _curiosty(self, input_):
        ...
        with tf.variable_scope('convnet'):
            conv_out = frame
            for i, (nf, rf, stride) in enumerate([(32, 8, 4), (32, 4, 2), (32, 3, 2), (32, 3, 1)]):
                # Downscale.
    ...
    def icm_unroll(self, actions, env_outputs):
        ...
        last_action = tf.reshape(one_hot_last_action, [-1, FLAGS.batch_size, self._num_actions])

        # ICM module inverse model
        concat_conv_out = tf.reshape(tf.concat([conv_out[:-1], conv_out[1:]], axis=-1), [-1, enc_dim * 2])
        fc_conv_out = snt.Linear(256, name='icm_inverse')(concat_conv_out)
        icm_inverse = tf.reshape(snt.Linear(self._num_actions, name='icm_logits')(fc_conv_out),
                                 [-1, FLAGS.batch_size, self._num_actions])

        # ICM module forward model (eta=0.01)
        concat_conv_out = tf.reshape(tf.concat([conv_out[:-1], last_action[1:]], axis=-1),
                                     [-1, enc_dim + self._num_actions])
        icm_forward = snt.Linear(256, name='icm_forward')(concat_conv_out)
        icm_forward = tf.reshape(tf.reduce_sum((snt.Linear(enc_dim, name='icm_output')(icm_forward) -
                                     tf.reshape(conv_out[1:], [-1, enc_dim])) ** 2, axis=-1) * 0.005,
                                 [-1, FLAGS.batch_size])
