...
class Agent(snt.RNNCore):
  ...
  def __init__(self, num_actions):
    ...
    with self._enter_variable_scope():
      ...
      # self._cells = [tf.contrib.rnn.LSTMBlockCell(n) for n in self._num_units]
      # self._core = tf.contrib.rnn.MultiRNNCell(self.cells)
  ...
  def _head(self, core_output):
    # Random normal noise
    nin = core_output.get_shape()[1].value
    w_noise = tf.random_normal(shape=[nin,  self._num_actions])
    b_noise = tf.random_normal(shape=[self._num_actions])

    # FC network weights
    w = tf.get_variable("w", [nin, self._num_actions],
                        initializer=tf.random_uniform_initializer(minval=-np.sqrt([3 / nin]), maxval=np.sqrt([3 / nin])))
    b = tf.get_variable("b", [self._num_actions],
                        initializer=tf.constant_initializer(value=0))

    # Noisy network weights
    w_sigma = tf.get_variable("w_sigma", [nin, self._num_actions],
                              initializer=tf.constant_initializer(0.017))
    b_sigma = tf.get_variable("b_sigma", [self._num_actions],
                              initializer=tf.constant_initializer(0.017))

    # Compute logits and baseline
    policy_logits = tf.matmul(core_output, w + tf.multiply(w_noise, w_sigma)) + b + tf.multiply(b_noise, b_sigma)
