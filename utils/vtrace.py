...
def from_importance_weights(
  ...
  values_rank = values.shape.ndims
  ...
  with tf.name_scope(name, values=[
    ...
    if values_rank > 2:
        ...
        discounts = tf.tile(tf.expand_dims(discounts, axis=-1), [1] * (values_rank - 1) + [shape[-1]])
        rewards = tf.tile(tf.expand_dims(rewards, axis=-1), [1] * (values_rank - 1) + [shape[-1]])
        rhos = tf.tile(tf.expand_dims(rhos, axis=-1), [1] * (values_rank - 1) + [shape[-1]])
    if clip_rho_threshold is not None:
